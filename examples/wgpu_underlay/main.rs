...
impl WGPUUnderlay {
    fn render(&mut self, surface_texture: &wgpu::Texture, rotation_enabled: bool) {
        ...
        .set_rendering_notifier(move |state, graphics_api| {
            ...
            match state {
                ...
                slint::RenderingState::BeforeRendering => {
                    let (surface_texture, config) = match graphics_api {
                        slint::GraphicsAPI::WGPU27 {
                    ...
                    println!("Calling underlay render with config {:?}", config);
                    if let (Some(underlay), Some(app)) = (underlay.as_mut(), app_weak.upgrade()) {
                        underlay.render(surface_texture, app.get_rotation_enabled());
